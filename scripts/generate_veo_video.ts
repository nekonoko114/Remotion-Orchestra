...
// Configuration
const PROJECT_ID =
  process.env.GOOGLE_CLOUD_PROJECT || 'remotion-orchestra-449300'; // Fallback or env
...
async function getAccessToken() {
  try {
    ...
    throw new Error(
      "Failed to get gcloud access token. Run 'gcloud auth login' first.",
    );
  }
}
  ...
  try {
    ...
    if (data.predictions && data.predictions[0]) {
      ...
      if (prediction.bytesBase64Encoded) {
        ...
        console.log(
          `Video URI returned: ${prediction.videoUri}. Downloading...`,
        );
        ...
        if (prediction.videoUri.startsWith('http')) {
          ...
          console.log(
            'Returned a GCS URI. Please ensure you have permissions or run: gsutil cp ' +
              prediction.videoUri +
              ' ' +
              outputPath,
          );
