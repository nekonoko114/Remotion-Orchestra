...
const getTikTokProfileWithPlaywright = async (browser: any, username: string) => {
    ...
    try {
        ...
        if (!avatarUrl) {
            ...
            const imgLocator = page.locator('img[alt*="profile"], img[src*="tiktokcdn"]');
        ...
        if (avatarUrl) {
            ...
            try {
                ...
                console.error(`  Failed to download avatar for ${username}:`, downloadErr);
                return { id: username, nickname: nickname, avatar: avatarUrl, localAvatar: "" };
            }
        }

        return { id: username, nickname: nickname || "No Avatar Found", avatar: "", localAvatar: "" };
...
const main = async () => {
  ...
  console.log(`Starting to fetch data for ${ids.length} users using Playwright...`);
