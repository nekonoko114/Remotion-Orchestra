...
const TimelineComponentSchema: z.ZodType<any> = z.lazy(() => z.object({
...
const ComponentMapper: React.FC<{
	...
	} else if (item.component && typeof EFFECT_REGISTRY[item.component] === "function") {
		const Comp = EFFECT_REGISTRY[item.component];
		const props = item.props || {};
		content = <Comp {...props} />;
	} else if (item.component && typeof TRANSITION_REGISTRY[item.component] === "function") {
	...
	return <AbsoluteFill style={item.style as React.CSSProperties}>{content}</AbsoluteFill>;
};
...
export const JsonVideoEngine: React.FC<{ data: any }> = ({ data: rawData }) => {
	...
	if (!result.success) {
		...
		return <AbsoluteFill style={{ backgroundColor: "red", color: "white" }}>Invalid JSON Data</AbsoluteFill>;
	}
	...
	return (
		<AbsoluteFill style={{ backgroundColor: "#000" }}>
			...
			{timeline.map((scene, index) => {
				...
				const endFrame = scene.end_frame ?? (startFrame + (scene.duration ?? 150));
				const duration = endFrame - startFrame;

				const transitionName = scene.transition;
				const TransitionComp = transitionName ? TRANSITION_REGISTRY[transitionName] : null;
