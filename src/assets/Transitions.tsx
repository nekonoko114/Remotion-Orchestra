 ...
 */
export const glitchSlideTransition = (
  ...
  return {
    component: ({ children, presentationProgress }: TransitionProps) => {
      ...
      const glitch =
        presentationProgress > 0.4 && presentationProgress < 0.6
          ? (Math.random() - 0.5) * 50
          : 0;

      const transform =
        direction === 'left'
          ? `translateX(${-offset + glitch}%)`
          : direction === 'right'
            ? `translateX(${offset + glitch}%)`
            : direction === 'top'
        ...
        <AbsoluteFill>
          <AbsoluteFill
            style={{
              transform,
              filter:
                presentationProgress > 0.3
                  ? `hue-rotate(${presentationProgress * 90}deg)`
                  : 'none',
            }}
          >
            {exiting}
      ...
      return (
        <AbsoluteFill>
          ...
          >
            <AbsoluteFill
              style={{
                transform: `translateX(${(1 - presentationProgress) * 20}px)`,
                opacity: 0.5,
          ...
          >
            ...
            <AbsoluteFill
              style={{
                transform: `translateX(${-split}px)`,
                opacity: 0.5,
                filter: 'sepia(1) saturate(10) hue-rotate(240deg)',
              }}
            >
              {exiting}
            </AbsoluteFill>
          ...
          >
            <div
              style={{
                ...
                height: 200,
                background:
                  'radial-gradient(circle, #fff 0%, #00f0ff 30%, transparent 70%)',
    ...
    component: ({ children, presentationProgress }: TransitionProps) => {
      ...
      return (
        <AbsoluteFill>
          <AbsoluteFill
            style={{
              filter: pixelSize > 0 ? `url(#pixelate-${pixelSize})` : 'none',
            }}
          >
...
export const starBurstTransition = (): TransitionPresentation<any> => {
  return {
    component: ({ children, presentationProgress }: TransitionProps) => {
      ...
      const scale = interpolate(presentationProgress, [0, 1], [0, 10], {
  ...
  return {
    component: ({ children, presentationProgress }: TransitionProps) => {
      ...
      return (
        <AbsoluteFill>
          <AbsoluteFill>{entering}</AbsoluteFill>
          <AbsoluteFill style={{ clipPath: `inset(0 0 ${shutter}% 0)` }}>
            {exiting}
          </AbsoluteFill>
          <AbsoluteFill style={{ clipPath: `inset(${shutter}% 0 0 0)` }}>
            {exiting}
          </AbsoluteFill>
        </AbsoluteFill>
      );
 ...
 * 12. MotionBlurPush
 */
export const motionBlurPushTransition = (
  ...
  return {
    component: ({ children, presentationProgress }: TransitionProps) => {
      ...
      return (
        <AbsoluteFill>
          <AbsoluteFill
            style={{ transform: exitTransform, filter: `blur(${blur}px)` }}
          >
            {exiting}
          </AbsoluteFill>
          <AbsoluteFill
            style={{ transform: enterTransform, filter: `blur(${blur}px)` }}
          >
            {entering}
          </AbsoluteFill>
        </AbsoluteFill>
      ...
      return (
        <AbsoluteFill>
          <AbsoluteFill style={{ opacity: 1 - presentationProgress }}>
            <AbsoluteFill style={{ clipPath: `inset(0 ${50 + split}% 0 0)` }}>
              {exiting}
            </AbsoluteFill>
            <AbsoluteFill style={{ clipPath: `inset(0 0 0 ${50 + split}%)` }}>
              {exiting}
            </AbsoluteFill>
            <AbsoluteFill
              style={{ opacity: 0.5, filter: 'hue-rotate(180deg)' }}
            >
              {exiting}
            </AbsoluteFill>
          </AbsoluteFill>
