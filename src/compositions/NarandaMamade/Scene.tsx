...
export const Scene: React.FC<SceneProps> = ({
  ...
  const entranceZoom = interpolate(frame, [0, 60], [1.15, 1], {
    extrapolateRight: 'clamp',
  });
  const exitScale = interpolate(frame, [duration - 15, duration], [1, 1.1], {
    extrapolateLeft: 'clamp',
  });
  const transitionScale = interpolate(frame, [0, 20], [1.2, 1], {
    extrapolateRight: 'clamp',
  });
  const chorusScale = isChorus
    ? interpolate(frame % 90, [0, 45, 90], [1, 1.02, 1])
    : 1;
  const impactScale =
    index % 4 === 0
      ? interpolate(frame, [0, 10], [1.05, 1], { extrapolateRight: 'clamp' })
      : 1;
  ...
  const entranceRotation =
    index % 2 === 0
      ? interpolate(frame, [0, 40], [2, 0], { extrapolateRight: 'clamp' })
      : 0;
  ...
  const exitRipple = interpolate(frame, [duration - 15, duration], [0, 20], {
    extrapolateLeft: 'clamp',
  });

  // 5. Filter Effects
  const brightness = interpolate(frame, [0, 20], [1.2, 1], {
    extrapolateRight: 'clamp',
  });
  const sparkle = 1 + Math.sin(frame * 0.1) * 0.1; // Saturation sparkle
  const bokehBlur = interpolate(frame, [0, 10], [10, 0], {
    extrapolateRight: 'clamp',
  });
  const exitBlur = interpolate(frame, [duration - 10, duration], [0, 5], {
    extrapolateLeft: 'clamp',
  });
  const hueRotation = Math.sin(frame * 0.01) * 10; // Subtle color shift
  const exitHueRotation = interpolate(
    frame,
  ...
  return (
    <AbsoluteFill
      style={{
        backgroundColor:
          isEpilogue || transparentBackground ? 'transparent' : 'black',
        overflow: 'hidden',
      }}
    >
      ...
      {isEpilogue && (
        <div
          style={{
            ...
            background:
              'conic-gradient(from 0deg, transparent 0deg, rgba(255, 255, 255, 0.1) 20deg, transparent 40deg, rgba(255, 200, 255, 0.1) 60deg, transparent 80deg, rgba(255, 255, 255, 0.1) 100deg, transparent 120deg, rgba(255, 200, 255, 0.1) 140deg, transparent 160deg)',
            zIndex: 0,
            opacity: 0.8 * opacity,
            pointerEvents: 'none',
          }}
      ...
      <div
        style={{
          ...
          boxShadow: isEpilogue
            ? '0 30px 60px rgba(0,0,0,0.8), 0 0 100px rgba(255, 182, 193, 0.8)'
            : 'none',
          overflow: 'hidden',
          backgroundColor: 'transparent',
          WebkitMaskImage: transparentBackground
            ? 'radial-gradient(circle, white 60%, transparent 100%)'
            : 'none',
          maskImage: transparentBackground
            ? 'radial-gradient(circle, white 60%, transparent 100%)'
            : 'none',
        }}
      >
      ...
      <AbsoluteFill style={{ pointerEvents: 'none', zIndex: 3 }}>
        {[...Array(8)].map((_, i) => {
          ...
          const flareOpacity =
            Math.sin(flarePhase * Math.PI) * (0.5 + beatPulse * 0.3);
          ...
          return (
            <div
              ...
              style={{
                ...
                height: `${flareSize}px`,
                background:
                  'radial-gradient(circle, rgba(255, 255, 255, 1) 0%, rgba(255, 200, 255, 0.5) 40%, transparent 70%)',
                borderRadius: '50%',
      ...
      <AbsoluteFill
        style={{
          ...
          pointerEvents: 'none',
          boxShadow: isEpilogue
            ? 'inset 0 0 200px rgba(255, 105, 180, 0.5)'
            : 'inset 0 0 150px rgba(255, 153, 204, 0.4)',
          opacity,
        }}
      />
              ...
              >
                {isHeart ? (
                  <svg
                    viewBox="0 0 32 32"
                    fill="#ff99cc"
                    style={{ filter: 'drop-shadow(0 0 3px #ff99cc)' }}
                  >
