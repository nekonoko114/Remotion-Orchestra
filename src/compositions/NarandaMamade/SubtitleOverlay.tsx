...
const CinematicChar: React.FC<{
  ...
  const blur = interpolate(frame - delay, [0, 20], [5, 0], {
    extrapolateRight: 'clamp',
  });
  const scale = interpolate(frame - delay, [0, 60], [1.1, 1], {
    extrapolateRight: 'clamp',
  });
...
const PopChar: React.FC<{
  ...
  const growthScale = interpolate(growthProgress, [0, 5], [1, 1.2], {
    extrapolateRight: 'clamp',
  });
        ...
        >
          {segments.map((segment, sIdx) => {
            ...
            return (
              <span
                key={sIdx}
                style={{ display: 'inline-block', margin: segmentMargin }}
              >
                {segment.split('').map((c, i) => {
                  ...
                  for (let k = 0; k < sIdx; k++)
                    globalCharIndex += segments[k].length;
                  globalCharIndex += i;
