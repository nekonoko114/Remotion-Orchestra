...
const FireAura: React.FC<{ color: string; pulse: number }> = ({
  color,
  pulse,
}) => {
    ...
    <AbsoluteFill style={{ pointerEvents: 'none', zIndex: 999 }}>
      ...
      <div
        style={{
          ...
          height: '100%',
          background:
            'linear-gradient(rgba(18, 10, 10, 0) 50%, rgba(20, 0, 0, 0.2) 50%)',
...
const FlyingShards: React.FC<{ frame: number }> = ({ frame }) => {
  const shards = Array.from({ length: 12 }).map((_, i) => {
    ...
    const distance = interpolate(frame, [0, 15], [20, 1000], {
...
const InfernoCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      >
        <AbsoluteFill
          style={{ transform: 'scale(1.5) rotate(45deg)', opacity: 0.5 }}
        >
...
export const BattleInferno: React.FC = () => {
  ...
  const beatRotation = interpolate(
    pulse,
    [0, 1],
    [0, (random(frame) - 0.5) * 3],
  );
  const invertAmount = interpolate(pulse, [0.85, 1], [0, 0.5], {
    ...
    >
              ...
              >
                <Img
                  ...
                  style={{
                    ...
                    objectFit: 'cover',
                    filter:
                      'invert(0.2) sepia(1) saturate(5) hue-rotate(-20deg)',
                  }}
                />
              </AbsoluteFill>
            </>
      ...
      <Sequence from={startVs} durationInFrames={VS_RULES_DURATION}>
        ...
        >
          ...
          >
            {['SURVIVAL', 'NO MERCY', 'HEAT LIMIT', 'FINAL IMPACT'].map(
              (rule, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background:
                        'linear-gradient(90deg, rgba(255,0,0,0.1), rgba(255,100,0,0.3), rgba(255,0,0,0.1))',
                      padding: '20px 80px',
                      borderRadius: 4,
      ...
      <Sequence from={startEnd} durationInFrames={END_DURATION}>
        ...
        >
          <AbsoluteFill
            style={{ zIndex: -1, opacity: 0.8, filter: 'hue-rotate(-120deg)' }}
          >
            <LensFlare />
          </AbsoluteFill>
          <div
            ...
            >
              <Img
                ...
                style={{
                  width: 600,
                  filter:
                    'drop-shadow(0 0 50px #ff0000) sepia(1) saturate(10) hue-rotate(-30deg)',
                }}
              />
            </div>
            <div
