...
import { WaterBattleBackground } from '../../components/backgrounds/WaterBattleBackground';
...
const BubbleAura: React.FC<{ color: string; pulse: number }> = ({
  color,
  pulse,
}) => {
    ...
    <AbsoluteFill style={{ pointerEvents: 'none', zIndex: 999 }}>
      ...
      <div
        style={{
          ...
          height: '100%',
          background:
            'radial-gradient(circle, transparent 20%, rgba(0, 100, 255, 0.15) 100%)',
      ...
      <svg style={{ position: 'absolute', width: 0, height: 0 }}>
        <filter id="liquid-distortion">
          <feTurbulence
            ...
            numOctaves="3"
            seed={Math.floor(useCurrentFrame() / 2)}
          >
            <animate
...
const WaterSplashShards: React.FC<{ frame: number }> = ({ frame }) => {
  const shards = Array.from({ length: 15 }).map((_, i) => {
    ...
    const distance = interpolate(frame, [0, 20], [0, 700], {
...
const WaterCard: React.FC<{
  ...
  // Visual Parameters
  const scale = interpolate(bounceSpr, [0, 1], [0.4, 1], {
  ...
  // Movement
  const floatY =
    Math.sin(frame / 30 + (direction === 'left' ? 0 : Math.PI)) * 25;
  const orbitalX = Math.cos(frame / 40) * 15;
  const rotation = interpolate(
    entrySpr,
    [0, 1],
    [direction === 'left' ? -15 : 15, 0],
  );
    ...
    >
      ...
      >
        ...
        <AbsoluteFill
          style={{
            background:
              'linear-gradient(to bottom, rgba(255,255,255,0.4), transparent 50%, rgba(0,0,0,0.3) 100%)',
            zIndex: 3,
          }}
        />
      </div>
...
const WaterWaveTransition: React.FC<{
  frame: number;
...
const VsClashAura: React.FC<{ pulse: number; frame: number }> = ({
  pulse,
  frame,
}) => {
    ...
    >
      ...
      <Sequence from={startMatchup} durationInFrames={MATCHUP_DURATION}>
        <AbsoluteFill>
          ...
          <AbsoluteFill
            style={{
              transform:
                frame - startMatchup >= 10 && frame - startMatchup < 20
          ...
          >
            ...
            >
              <WaterCard
                ...
                pulse={softPulse}
              />
              {frame - startMatchup === 5 && (
                <ImpactShockwave color="#00E5FF" />
              )}
            </div>
            ...
            >
              <WaterCard
                ...
                pulse={softPulse}
              />
              {frame - startMatchup === 15 && (
                <ImpactShockwave color="#1DE9B6" />
              )}
            </div>
          </AbsoluteFill>
        </AbsoluteFill>
      ...
      <Sequence from={startVs} durationInFrames={VS_RULES_DURATION}>
        ...
        >
          ...
          >
            {['NO BUBBLES', 'DEEP DIVE', 'PEARL HUNT', 'TIDAL WAVE'].map(
              (rule, i) => {
      ...
      <Sequence from={startEnd} durationInFrames={END_DURATION}>
        ...
        >
          ...
          >
            <div
              style={{
                opacity: interpolate(frame - startEnd, [0, 10], [0, 1]),
              }}
            >
              <p
                style={{
            ...
            >
              <Img
                ...
                style={{ width: 600, filter: 'drop-shadow(0 0 30px #00E5FF)' }}
              />
            </div>
            <div
              style={{
