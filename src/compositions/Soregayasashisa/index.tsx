...
export const SoregayasashisaMV: React.FC = () => {
    ...
    const transitionFrames: number[] = LYRIC_GROUPS
        .map((g) => {
    ...
    return (
        <AbsoluteFill style={{ backgroundColor: '#0f0f1f' }}>
            ...
            <AbsoluteFill style={{ zIndex: 10 }}>
                <TransitionSeries>
                    {/* イントロ・待機シーン (scene_0) */}
                    <TransitionSeries.Sequence durationInFrames={LYRICS[0].startFrame + TRANSITION_DURATION}>
                        <Scene 
                    ...
                    {LYRIC_GROUPS.map((group, index) => {
                        ...
                        const sequenceDuration = isLast ? baseDuration : baseDuration + TRANSITION_DURATION;
                            ...
                            <React.Fragment key={index}>
                                ...
                                {!isLast && (
                                    <TransitionSeries.Transition
                                        presentation={(() => {
                                            const type = index % 8;
                                            if (type === 0) return slide({ direction: 'from-right' });
                                            if (type === 1) return wipe({ direction: 'from-bottom' });
                                            if (type === 2) return clockWipe({ width, height });
                                            if (type === 3) return flip({ direction: 'from-top' });
                                            if (type === 4) return slide({ direction: 'from-left' });
