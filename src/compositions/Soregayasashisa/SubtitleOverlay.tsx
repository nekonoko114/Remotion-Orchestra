...
import { useCurrentFrame, AbsoluteFill, Sequence, interpolate, useVideoConfig, Easing } from 'remotion';
import { gsap } from 'gsap';
import { LYRICS } from './lyrics';

// --- 4. Handwritten Style (Intro/A-Melo Variation) ---
// ペンで書いているような「削り出し」出現。ゆらゆら揺れる。
const HandwrittenLyric: React.FC<{ text: string; duration: number }> = ({ text, duration }) => {
    ...
    return (
        <div style={{
            ...
            {text.split('').map((char, i) => {
                ...
                const opacity = interpolate(frame, [charStart, charStart + 2], [0, 1], { extrapolateRight: 'clamp' });
...
const SeishunPlayfulLyric: React.FC<{ text: string; duration: number }> = ({ text, duration }) => {
    ...
    useEffect(() => {
        const link = document.createElement('link');
        link.href = 'https://fonts.googleapis.com/css2?family=M+PLUS+Rounded+1c:wght@800;900&display=swap';
        link.rel = 'stylesheet';
        document.head.appendChild(link);
        return () => {};
    }, []);
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            const vibrantColors = ["#FF4757", "#2ED573", "#FFA502", "#1E90FF", "#3742fa"];

            // 1. 登場（Appear）：わんぱくバウンス
            tl.fromTo(".playful-char", {
                scale: 0,
                opacity: 0,
                y: 100,
    ...
    const charPositions = useMemo(() => {
        ...
        return text.split('').map((_, i) => {
            ...
            const charsInThisRow = Math.min(MAX_CHARS_PER_ROW, text.length - (row * MAX_CHARS_PER_ROW));
            ...
            {text.split('').map((char, i) => (
                ...
                >
                    ...
                    <span style={{
                        ...
                        zIndex: -1,
                        color: 'transparent',
                        backgroundImage: 'repeating-linear-gradient(90deg, #091e59 0px, #091e59 4px, transparent 4px, transparent 8px)',
                    ...
                    <span className="front-char" style={{
                        position: 'relative',
                        zIndex: 1,
                        fontFamily: '"M PLUS Rounded 1c", sans-serif',
                        fontSize: '110px',
...
// 霧の中から現れ、霧の中に消える
const BlurDissolveLyric: React.FC<{ text: string; duration: number }> = ({ text, duration }) => {
    const frame = useCurrentFrame();
    
    const blur = interpolate(frame, [0, 20, duration - 20, duration], [20, 0, 0, 20]);
    const scale = interpolate(frame, [0, 20, duration - 20, duration], [1.2, 1, 1, 0.8]);
...
const SketchRevealLyric: React.FC<{ text: string; duration: number }> = ({ text }) => {
    const frame = useCurrentFrame();
    ...
    return (
        <div style={{
            ...
            {text.split('').map((char, i) => {
                ...
                // 1. アウトラインの浮き出し (0〜15f)
                const outlineBlur = interpolate(progress, [0, 10], [10, 0], { extrapolateRight: 'clamp' });
                const outlineOpacity = interpolate(progress, [0, 10, 20], [0, 0.8, 1], { extrapolateRight: 'clamp' });
