 ...
 */
export const GlitchDistortion: React.FC<{ frequency?: number }> = ({ frequency = 0.1 }) => {
    const frame = useCurrentFrame();
 ...
 */
export const SpeedLines: React.FC<{ color?: string }> = ({ color = "rgba(255,255,255,0.2)" }) => {
    const frame = useCurrentFrame();
...
export const NeonGrid: React.FC = () => {
    ...
    return (
        <AbsoluteFill style={{ pointerEvents: "none", perspective: "500px", overflow: "hidden" }}>
            <div style={{
                position: "absolute",
                bottom: "-50%",
                left: "-50%",
...
export const ChromaticAberrationOverlay: React.FC = () => {
    return (
        <AbsoluteFill style={{ pointerEvents: "none" }}>
            <AbsoluteFill style={{ filter: "matrix(1,0,0,1,2,0)", opacity: 0.3, mixBlendMode: "screen" }} />
            <AbsoluteFill style={{ filter: "matrix(1,0,0,1,-2,0)", opacity: 0.3, mixBlendMode: "screen" }} />
        </AbsoluteFill>
    );
};
 ...
 */
export const FlashPulse: React.FC<{ pulse: number, color?: string }> = ({ pulse, color = "#fff" }) => {
    return (
        <AbsoluteFill style={{
            pointerEvents: "none",
            backgroundColor: color,
 ...
 */
export const RadialBlurImpact: React.FC<{ intensity: number }> = ({ intensity }) => {
    return (
        <AbsoluteFill style={{
            pointerEvents: "none",
...
export const BokehOverlay: React.FC = () => {
    const frame = useCurrentFrame();
    const dots = useMemo(() => Array.from({ length: 20 }).map((_, i) => ({
        id: i,
        x: Math.random() * 100,
        y: Math.random() * 100,
    ...
    return (
        <AbsoluteFill style={{ pointerEvents: "none", opacity: 0.2 }}>
            <svg width="100%" height="100%" viewBox="0 0 1000 1000" preserveAspectRatio="none">
                <path d={`M 0 500 Q 250 ${500 + Math.sin(frame/10) * 200} 500 500 T 1000 500`} fill="none" stroke="#00f0ff" strokeWidth="2" />
                <path d={`M 0 550 Q 250 ${550 + Math.cos(frame/12) * 150} 500 550 T 1000 550`} fill="none" stroke="#ff00ff" strokeWidth="1" />
            </svg>
        </AbsoluteFill>
    );
};
 ...
 */
export const GlitchBarsStatic: React.FC<{ frequency: number }> = ({ frequency }) => {
    const bars = Math.random() < frequency ? Array.from({ length: 3 }).map(() => ({
        top: Math.random() * 100,
