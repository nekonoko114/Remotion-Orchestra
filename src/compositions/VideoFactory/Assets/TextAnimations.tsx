 ...
 */
export const WordSlideUp: React.FC<{ text: string, color?: string, delay?: number }> = ({ text, color = "white", delay = 0 }) => {
 ...
 */
export const LetterBounce: React.FC<{ text: string, color?: string }> = ({ text, color = "white" }) => {
    const frame = useCurrentFrame();
    ...
    return (
        <div style={{ display: "flex", justifyContent: "center" }}>
            {letters.map((l, i) => {
                ...
                const opacity = interpolate(frame - i * 2, [0, 5], [0, 1], { extrapolateRight: "clamp" });
                return (
                    <span key={i} style={{
                        display: "inline-block",
 ...
 */
export const NeonGlowText: React.FC<{ text: string, color?: string }> = ({ text, color = "#00f0ff" }) => {
    const frame = useCurrentFrame();
...
export const GlitchText: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div style={{ position: "relative", fontSize: "100px", fontWeight: "900", color: "white" }}>
            <div style={{ position: "absolute", left: offset, top: 0, color: "#ff00ff", opacity: active ? 0.7 : 0 }}>{text}</div>
            <div style={{ position: "absolute", left: -offset, top: 0, color: "#00ffff", opacity: active ? 0.7 : 0 }}>{text}</div>
...
export const TypingEffect: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div style={{ fontFamily: "monospace", fontSize: "40px", color: "#00f0ff" }}>
            {visibleText}
            {frame % 10 < 5 && "_"}
        </div>
    );
...
export const ScaleReveal: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div style={{ transform: `scale(${scale})`, opacity: scale, textAlign: "center", fontSize: "120px", fontWeight: "bold", color: "white" }}>
            {text}
        </div>
    );
};
 ...
 */
export const BlurReveal: React.FC<{ text: string, duration?: number }> = ({ text, duration = 30 }) => {
    const frame = useCurrentFrame();
    const blur = interpolate(frame, [0, duration], [20, 0], { extrapolateRight: "clamp" });
    const opacity = interpolate(frame, [0, duration], [0, 1], { extrapolateRight: "clamp" });

    return (
        <div style={{ filter: `blur(${blur}px)`, opacity, textAlign: "center", fontSize: "80px", color: "white" }}>
            {text}
        </div>
    );
};
...
export const StaggeredSlideIn: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div style={{ display: "flex", justifyContent: "center" }}>
            {text.split("").map((l, i) => {
                const s = spring({ frame: frame - i * 2, fps, config: { damping: 12 } });
                return (
                    <span key={i} style={{
                        display: "inline-block",
...
export const ElasticPop: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div style={{ transform: `scale(${scale})`, textAlign: "center", fontSize: "90px", color: "#ff00ff" }}>
            {text}
        </div>
    );
};
 ...
 */
export const ShadowPulse: React.FC<{ text: string, color?: string }> = ({ text, color = "#ff00ff" }) => {
    const frame = useCurrentFrame();
...
export const BorderDrawReveal: React.FC<{ text: string }> = ({ text }) => {
    const frame = useCurrentFrame();
    const progress = interpolate(frame, [0, 20], [0, 100], { extrapolateRight: "clamp" });
    return (
...
export const GlitchExplosion: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <div style={{ display: "flex", justifyContent: "center", opacity: 1 - progress }}>
            {text.split("").map((l, i) => (
                <span key={i} style={{
                    display: "inline-block",
