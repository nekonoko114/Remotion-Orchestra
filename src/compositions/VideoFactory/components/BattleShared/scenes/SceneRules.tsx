...
export const SceneRules: React.FC<{ theme: BattleSpiritTheme }> = ({ theme }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: theme.themeColor === '#e0f7fa' ? 'transparent' : '#040000', overflow: 'hidden' }}>
      <SvgDefs frame={frame} />
      {rulesImpact > 0.8 && <div style={{ position: 'absolute', inset: 0, backgroundColor: 'white', opacity: rulesImpact * 0.8, zIndex: 10 }} />}
      {new Array(30).fill(0).map((_, i) => <Particle key={i} seed={i * 19 + 800} frame={frame} color={i % 2 === 0 ? theme.particleColor1 : theme.particleColor2} direction={theme.themeColor === '#e0f7fa' ? 'down' : 'up'} />)}
