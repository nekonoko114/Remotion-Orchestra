...
const Sparkle: React.FC<{ seed: number; frame: number }> = ({ seed, frame }) => {
	const { height, width } = useVideoConfig();
	const x = random(seed) * width;
	const y = random(seed + 1) * height;
	const size = interpolate(random(seed + 2), [0, 1], [10, 30]);
	const opacity = interpolate(Math.sin(frame / 10 + random(seed) * 10), [-1, 1], [0, 0.8]);
...
export const KawaiiBackground: React.FC = () => {
	...
	return (
		<AbsoluteFill style={{ 
			...
			{/* Glossy Overlay */}
			<AbsoluteFill style={{
				background: "radial-gradient(circle at 50% 50%, transparent 60%, rgba(255, 255, 255, 0.3) 100%)",
