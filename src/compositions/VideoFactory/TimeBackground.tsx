...
export const TimeBackground: React.FC<Props> = ({
    ...
    if (icons) {
      ...
      musicParticles.forEach((p) => {
        ...
        const symbolIndex = Math.floor(
          random(`morph-${timeSeed}-${p.seed}`) * (SPRITE_ROWS * SPRITE_COLS),
        );
        ...
        ctx.globalAlpha =
          (0.55 + Math.sin(frame * 0.03 + p.floatOffset) * 0.1) *
          (1 + pulse * 0.01);
  ...
  return (
    <AbsoluteFill>
      ...
      <AbsoluteFill
        style={{ top: 'auto', height: 300, bottom: 50, opacity: 0.6 }}
      >
        {' '}
        {/* Increased opacity */}
        <div
        ...
        >
          {Array.from({ length: 40 }).map((_, i) => {
            const h =
              interpolate(Math.sin(frame * 0.1 + i), [-1, 1], [15, 100]) *
              (1 + pulse * 0.02);
            return (
              <div
                key={i}
      ...
      <AbsoluteFill
        style={{
          background:
            'linear-gradient(to bottom, rgba(208,0,255,0.03) 50%, transparent 50%)',
