...
const EnergyAura: React.FC<{ color: string; pulse: number }> = ({ color, pulse }) => {
...
const FlyingShards: React.FC<{ frame: number }> = ({ frame }) => {
	const shards = Array.from({ length: 8 }).map((_, i) => {
		...
		const distance = interpolate(frame, [0, 15], [0, 800], { easing: Easing.out(Easing.quad) });
...
const CrystalCard: React.FC<{
	...
	const scale = interpolate(spr, [0, 1], [0.5, 1], { easing: Easing.out(Easing.back(1.2)) });
	const slide = interpolate(spr, [0, 1], [direction === "left" ? -500 : 500, 0]);
...
export const BattleCrystal: React.FC = () => {
	...
	const beatRotation = interpolate(pulse, [0, 1], [0, (random(frame) - 0.5) * 2]);

	// NEW: Color Inversion pulse (Heavy flash on beat)
	const invertAmount = interpolate(pulse, [0.8, 1], [0, 0.4], { extrapolateLeft: "clamp" });
	...
	return (
		<AbsoluteFill style={{ 
			...
			<Sequence from={startJol} durationInFrames={JOL_INTRO_DURATION}>
				<AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
					<CrystalCard player={MOCK_JOL_PLAYER} frame={frame - startJol} direction="left" pulse={pulse} />
				</AbsoluteFill>
			</Sequence>
			...
			<Sequence from={startOpponent} durationInFrames={OPPONENT_INTRO_DURATION}>
				<AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
					<CrystalCard player={MOCK_OPPONENT} frame={frame - startOpponent} direction="right" pulse={pulse} />
				</AbsoluteFill>
			</Sequence>
					...
					*/}
					<AbsoluteFill style={{
						...
						<div style={{ 
							...
							<CrystalCard player={MOCK_JOL_PLAYER} frame={frame - startMatchup} direction="left" pulse={pulse} />
						...
						<div style={{ 
							...
							<CrystalCard player={MOCK_OPPONENT} frame={frame - startMatchup} direction="right" pulse={pulse} />
			...
			<Sequence from={startVs} durationInFrames={VS_RULES_DURATION}>
				<AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
					<div style={{
						...
						<h2 style={{ fontSize: 110, color: "#00f0ff", fontFamily: "Impact", margin: "10px 0 0 0" }}>2026.01.31 22:00</h2>
					</div>
					...
					<div style={{ 
						...
						{["GLOVE", "NO BOOSTER", "FREE STYLE", "ONE SHOT"].map((rule, i) => {
							...
							return (
								<div key={rule} style={{
									...
									padding: "20px 80px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.2)", width: 750, textAlign: "center",
