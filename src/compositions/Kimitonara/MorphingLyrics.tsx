...
export const MorphingLyrics: React.FC<MorphingLyricsProps> = ({
  text,
  startTime,
  endTime,
  syncOffset,
  ...
  const paths = useMemo(() => {
    ...
    const fullFontSize = Math.min(
      width * 0.1,
      (width * 0.7) / (maxCharsPerLine * 1.0),
    );
    ...
    const startPathObj = font.getPath(
      firstNonEmptyChar || ' ',
      0,
      0,
      baseFontSize,
...
const MorphingChar: React.FC<MorphingCharProps> = ({
  ...
  const finalStrokeColor = interpolateColors(
    progress,
    [0, 1],
    [softLavender, sunsetOrange],
  );
  ...
  const jitterIntensity = isMorphing
    ? interpolate(progress, [0, 0.8, 1], [15, 5, 0])
    : 0;
  const jitterX =
    Math.sin(currentRelativeFrame * 0.8 + index) * jitterIntensity;
  const jitterY =
    Math.cos(currentRelativeFrame * 1.2 + index) * jitterIntensity;
  ...
  // Base Position
  const currentX =
    interpolate(progress, [0, 1], [startOffset.x, endOffset.x]) + jitterX;
  const currentY =
    interpolate(progress, [0, 1], [startOffset.y, endOffset.y]) + jitterY;
  ...
  return (
    ...
    >
      ...
      <defs>
        ...
        <path ref={pathRef} id={`char-geom-${index}`} d={startPath} />
      </defs>
