...
export const ChorusLyrics: React.FC<ChorusLyricsProps> = ({ subtitles, layer = 'front' }) => {
...
const KineticStripText: React.FC<{
    ...
    scaleMult: number;
    isBack: boolean;
}> = ({ text, relativeFrame, width, height, index, parallaxMultiplier, opacity, blur, scaleMult, isBack }) => {
    ...
    const currentX = interpolate(progress, [0, 1], [startX * parallaxMultiplier, 0]);
                ...
                {/* Text Container */}
                 <div style={{
                    ...
                    {chars.map((char, i) => {
                        ...
                        const charOpacity = interpolate(charFrame, [0, 3], [0, 1], { extrapolateRight: 'clamp' });
