...
import { useCurrentFrame, useVideoConfig, AbsoluteFill, interpolate, Easing, Sequence, spring } from 'remotion';
import { z } from 'zod';
...
export const isChorusTime = (time: number) => {
    // 厳密な判定: 範囲内かどうか
    return CHORUS_RANGES.some(range => time >= range.start && time <= range.end);
};
...
export const Lyrics: React.FC<LyricsProps> = ({ subtitles, layer }) => {
    ...
    const isCurrentChorus = currentSubtitle ? isChorusTime(currentSubtitle.start) : false;
    ...
    return (
        <AbsoluteFill ref={containerRef} style={{ pointerEvents: 'none' }}>
            ...
            {subtitles.map((sub, index) => {
                ...
                const subLayer = isChorus ? 'foreground' : (index % 2 === 0 ? 'background' : 'foreground');
...
const NonChorusLyric: React.FC<{ text: string, durationFrames: number, fps: number, index: number }> = ({ text, durationFrames, fps, index }) => {
    const frame = useCurrentFrame();
    ...
    const glowColor = pseudoRandom(index * 111.1) > 0.5 ? 'rgba(177, 156, 217, 0.8)' : 'rgba(255, 158, 125, 0.8)';
    ...
    } else if (animType === 2) { // Pass Through / Drift
        const driftDir = pseudoRandom(index * 333) > 0.5 ? 1 : -1;
        const driftStart = -50 * driftDir;
        const driftEnd = 50 * driftDir;
        const driftX = interpolate(frame, [0, durationFrames], [driftStart, driftEnd]);
    ...
    return (
        <div className={`lyric-row-${index}`} style={containerStyle}>
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', width: '90%' }}>
                {chars.map((char, charIndex) => {
                    ...
                    if (animType === 0) { // Pop / Scatter (Original)
                         opacity = interpolate(charFrame, [0, 5], [0, 1], { extrapolateRight: 'clamp' });
                         scale = interpolate(charFrame, [0, 10], [3.0, 1], { extrapolateRight: 'clamp', easing: Easing.out(Easing.exp) });
                         blur = interpolate(charFrame, [0, 5], [20, 0], { extrapolateRight: 'clamp' });
                    } else if (animType === 1) { // Slide In (Letters match container)
                         opacity = interpolate(charFrame, [0, 5], [0, 1], { extrapolateRight: 'clamp' });
                         // Subtle individual float
                         charTransY = Math.sin((frame + charIndex) / 10) * 5;
                    } else if (animType === 2) { // Pass Through (Fade smoothly)
                         opacity = interpolate(charFrame, [0, 10], [0, 1], { extrapolateRight: 'clamp' });
                    }

                    // 2. Exit (Common)
                    const exitOpacity = interpolate(frame, [durationFrames - 8, durationFrames], [1, 0], { extrapolateRight: 'clamp' });
